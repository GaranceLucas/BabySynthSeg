...
for i in range(len(train_dataset1)):
    mri, seg = train_dataset1[i]
    seg_data = seg[0][0]
    binary_mask = convert_to_binary_mask(seg_data)
    seg_data = (binary_mask, seg[0][1])
    train_dataset1[i] = (mri, (seg_data, seg[1]))  # Keep the affine matrix and filename

for i in range(len(train_dataset2)):
    mri, seg = train_dataset2[i]
    seg_data = seg[0][0]
    binary_mask = convert_to_binary_mask(seg_data)
    seg_data = (binary_mask, seg[0][1])
    train_dataset2[i] = (mri, (seg_data, seg[1]))

for i in range(len(val_dataset1)):
    mri, seg = val_dataset1[i]
    seg_data = seg[0][0]
    binary_mask = convert_to_binary_mask(seg_data)
    seg_data = (binary_mask, seg[0][1])
    val_dataset1[i] = (mri, (seg_data, seg[1]))

for i in range(len(val_dataset2)):
    mri, seg = val_dataset2[i]
    seg_data = seg[0][0]
    binary_mask = convert_to_binary_mask(seg_data)
    seg_data = (binary_mask, seg[0][1])
    val_dataset2[i] = (mri, (seg_data, seg[1]))
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(config['num_epochs'])):
        ...
        for batch in zip(train_loader1, train_loader2):
            ...
            # adding a channel dimension to the x tensor
            x = x.unsqueeze(1)
