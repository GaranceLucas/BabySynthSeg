...
results_path_2D = ''   # Path to save the cropped data and segmentations for the 2D models
results_path_3D = ''   # Path to save the cropped data and segmentations for the 3D models
...
for file in data_files:
    data.append((nib.as_closest_canonical(nib.load(data_path + file)).get_fdata(), nib.as_closest_canonical(nib.load(data_path + file)).affine))
...
for i in tqdm(range(len(data))):
    ...
    segmentation_file = segmentation_path + 'segmentation_sub-' + file_number + '.nii.gz'  # segmentation_path + file_number + '_T2_Segmentation-label' + '.nii.gz'   # To modify depending on the file names
    ...
    nib.save(nib.Nifti1Image(cropped_padded_resampled_mri2, affine=affine_mri), results_path_2D + file_number + '_mri.nii.gz')
    nib.save(nib.Nifti1Image(cropped_padded_resampled_mri3, affine=affine_mri), results_path_3D + file_number + '_mri.nii.gz')
    
    nib.save(nib.Nifti1Image(cropped_padded_resampled_seg2, affine=affine_mri), results_path_2D + file_number + '_segmentation.nii.gz')
    nib.save(nib.Nifti1Image(cropped_padded_resampled_seg3, affine=affine_mri), results_path_3D + file_number + '_segmentation.nii.gz')